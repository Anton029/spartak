const picturesSection = document.querySelector('.categories_pictures')
const closeButton = document.querySelector('.gallery_popup .close_button')
const galleryPopup = document.querySelector('.gallery_popup')
const popupWrapper = document.querySelector('.gallery_popup .popup_content')
const slideMargin = window.getComputedStyle(document.querySelector('.slide')).marginRight
let tracker
let categoryType
let slideStep
let slidesAmount
...
picturesSection.addEventListener('click', (e) => {
    if(e.target.classList.contains('big_picture') || e.target.classList.contains('small_picture'))
    {
        categoryType = e.target.querySelector('.visible').classList[0];
        slideStep = e.target.dataset.picture;

        document.querySelector('body').style.cssText = 'overflow: hidden'

        galleryPopup.classList.add('visible')
        document.querySelector(`.slider_tracker[data-slider="${categoryType}"]`).classList.add('visible')

        slidesAmount =
        document.querySelector(`.slider_tracker[data-slider="${categoryType}"]`).querySelectorAll('.slide').length

        tracker = document.querySelector(`.slider_tracker[data-slider="${categoryType}"]`)
        tracker.style.cssText = `transform: translateX(calc( (-100% - ${slideMargin})  * ${slideStep} ) )`;
    }
})

popupWrapper.addEventListener('click', (e) => {
    if(e.target.classList.contains('right_arrow') && slideStep < slidesAmount - 1)
    {
        ++slideStep
        tracker.style.cssText = `transform: translateX(calc( (-100% - ${slideMargin})  * ${slideStep} ) )`;
    }
    if(e.target.classList.contains('left_arrow') && slideStep >= 1)
    {
...
closeButton.addEventListener('click', function (){
    ...
    document.querySelector(`.slider_tracker[data-slider="${categoryType}"]`).classList.remove('visible')
})


let foodSlideWidth = document.querySelector('.catering_gallery .slider_box').offsetWidth
const foodSliderPrev = document.querySelector('.catering_gallery .left_arrow')
const foodSliderNext = document.querySelector('.catering_gallery .right_arrow')
const foodSliderTracker = document.querySelector('.catering_gallery .slider_tracker')
const foodSlides = document.querySelectorAll('.catering_gallery .slider_tracker .slide')
let foodSlide = 1

let foodSlidesMargin = window.getComputedStyle(document.querySelectorAll('.content_5_wrapper .slide')[2]).marginRight

let foodSliderSlides
...
window.addEventListener("resize", () => {
    if (window.innerWidth >= 768){
        if (foodSlide != 3) {
            foodSlide = Math.ceil(foodSlide / 3)
        }
        else if(foodSlide == 3) {
            foodSlide = 1
        }
    ...
    foodSlidesMargin = window.getComputedStyle(document.querySelectorAll('.content_5_wrapper .slide')[2]).marginRight

    foodSlideWidth = document.querySelector('.catering_gallery .slider_box').offsetWidth
    ...
    foodSliderTracker.style.cssText = `transform: translateX(calc((-${foodSlideWidth}px - ${foodSlidesMargin}) * ${foodSlide - 1}))`
});


foodSliderPrev.addEventListener('click', () => {
    if (foodSlide > 1) {
        --foodSlide
        foodSliderTracker.style.cssText = `transform: translateX(calc((-${foodSlideWidth}px - ${foodSlidesMargin}) * ${foodSlide - 1}))`
    }
...
foodSliderNext.addEventListener('click', () => {
    if (foodSlide < foodSliderSlides) {
        ++foodSlide
        foodSliderTracker.style.cssText = `transform: translateX(calc((-${foodSlideWidth}px - ${foodSlidesMargin}) * ${foodSlide - 1}))`
    }
})
...
const foodPopup = document.querySelector('.food_popup')
const foodPopupPrev = document.querySelector('.food_popup .popup_content .left_arrow')
const foodPopupNext = document.querySelector('.food_popup .popup_content .right_arrow')
const foodPopupTracker = document.querySelector('.food_popup .slider_tracker')
const closeFoodPopup = document.querySelector('.food_popup .close_button')
let slideData = 1

foodSlides.forEach((e) => {
    e.addEventListener('click', () => {
        document.querySelector('body').style.cssText = 'overflow: hidden'
        slideData = e.dataset.sliderImg
        foodPopupTracker.style.cssText = `transform: translateX(calc((-100% - 400px) * ${slideData - 1}))`
        foodPopupTracker.classList.add('visible')
        foodPopup.classList.add('visible')
    })
})

foodPopupNext.addEventListener('click', () => {
    if (slideData < 9) {
        ++slideData
        foodPopupTracker.style.cssText = `transform: translateX(calc((-100% - 400px) * ${slideData - 1}))`
    }
...
foodPopupPrev.addEventListener('click', () => {
    if (slideData > 1) {
        --slideData
        foodPopupTracker.style.cssText = `transform: translateX(calc((-100% - 400px) * ${slideData - 1}))`
    }
...
closeFoodPopup.addEventListener('click', () => {
    ...
    document.querySelector('body').style.cssText = 'overflow-y: visible'
    foodPopup.classList.remove('visible')
    foodPopupTracker.classList.remove('visible')
})
